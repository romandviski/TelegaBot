...
class PositiveChanger(StatesGroup):
...
async def changer_start(message: types.Message):
    ...
    await PositiveChanger.waiting_for_boi.set()
...
async def shot_chosen(message: types.Message, state: FSMContext):
    ...
    set_positive(boi, shot)
    await message.answer(str(boi) + ", " + str(shot), reply_markup=types.ReplyKeyboardRemove())
    await message.answer("Статус изменён на ПРОВЕРЕН!")
    await message.answer("Ссылка на таблицу: ")
    await message.answer("https://docs.google.com/spreadsheets/xxx")
    await state.finish()

def register_handlers_positive(dp: Dispatcher):
    dp.register_message_handler(changer_start, commands="positive", state="*")
    dp.register_message_handler(boi_chosen, state=PositiveChanger.waiting_for_boi)
    dp.register_message_handler(shot_chosen, state=PositiveChanger.waiting_for_shot)
