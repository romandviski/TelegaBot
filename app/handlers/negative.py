...
class NegativeChanger(StatesGroup):
...
async def changer_start(message: types.Message):
    ...
    await NegativeChanger.waiting_for_boi.set()
...
def register_handlers_negative(dp: Dispatcher):
    dp.register_message_handler(changer_start, commands="negative", state="*")
    dp.register_message_handler(boi_chosen, state=NegativeChanger.waiting_for_boi)
    dp.register_message_handler(shot_chosen, state=NegativeChanger.waiting_for_shot)
